...
function custom_nav_update($menu_id, $menu_item_db_id, $args ) {
    if ( is_array($_REQUEST['menu-item-mega']) ) {
        $custom_value = $_REQUEST['menu-item-mega'][$menu_item_db_id];
        update_post_meta( $menu_item_db_id, '_menu_item_mega', $custom_value );
    }
}
...
function custom_nav_item($menu_item) {
    $menu_item->custom = get_post_meta( $menu_item->ID, '_menu_item_mega', true );
    return $menu_item;
}
...
function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0 ) {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = $item->title.__('(Invalid)','alaya');
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = $item->title.__('(Pending)','alaya');
    }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','alaya'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','alaya'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item','alaya'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php _e( 'Edit Menu Item','alaya' ); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_html($item_id); ?>">
            <?php if( 'custom' == $item->type ) : ?>
                <p class="field-url description description-wide">
                    <label for="edit-menu-item-url-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'URL','alaya'); ?><br />
                        <input type="text" id="edit-menu-item-url-<?php echo esc_html($item_id); ?>" class="widefat code edit-menu-item-url" name="menu-item-url[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->url ); ?>" />
                    </label>
                </p>
            <?php endif; ?>
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_html($item_id); ?>">
                    <?php _e( 'Navigation Label','alaya'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_html($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_html($item_id); ?>">
                    <?php _e( 'Title Attribute','alaya'); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo $item_id; ?>">
                    ...
                    <?php _e( 'Open link in a new window/tab','alaya' ); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_html($item_id); ?>">
                    <?php _e( 'CSS Classes (optional)','alaya'); ?><br />
            ...
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_html($item_id); ?>">
                    <?php _e( 'Link Relationship (XFN)','alaya' ); ?><br />
            ...
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_html($item_id); ?>">
                    ...
                    <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.','alaya'); ?></span>
                </label>
            </p>        
            ...
            <p class="field-custom description description-wide">
                <label for="edit-menu-item-mega-<?php echo esc_html($item_id); ?>">
                    <?php _e( 'Mega Menu Style','alaya'); ?><br />
                    <input type="radio" id="edit-menu-item-mega-<?php echo esc_html($item_id); ?>" class="widefat code edit-menu-item-mega" name="menu-item-mega[<?php echo esc_html($item_id); ?>]" value="wide" <?php if(esc_attr( $item->custom )=='wide'){echo 'checked';}?> /> <?php _e('Wide','alaya');?> &nbsp;
                    <input type="radio" id="edit-menu-item-mega-<?php echo esc_html($item_id); ?>" class="widefat code edit-menu-item-mega" name="menu-item-mega[<?php echo esc_html($item_id); ?>]" value="narrow" <?php if(esc_attr( $item->custom )=='narrow'){echo 'checked';}?> /> <?php _e('Narrow','alaya');?>
                </label>
                <br/> 
                <span class="description"><?php _e('Only affect on the category item.','alaya');?></span>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php _e('Original','alaya');?>: <a href="' . esc_attr( $item->url ) . '"><?php esc_html( $original_title );?></a>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php _e('Remove','alaya'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_html($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => esc_html($item_id), 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_html($item_id); ?>"><?php _e('Cancel','alaya'); ?></a>
            </div>
