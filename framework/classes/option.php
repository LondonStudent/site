...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', 'alaya' ),
			'menu_label'            => __( 'Vafpress Options', 'alaya' ),
			'priority'              => 10,
		), $configs);
		...
		else throw new Exception(__( 'Option Key is required', 'alaya' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', 'alaya' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', 'alaya' ), 1);
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'alaya'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", 'alaya');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", 'alaya');
				}
			}
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", 'alaya');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access", 'alaya');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'alaya' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', 'alaya'));
			...
			$restore_section->set_title(__('Restore Default', 'alaya'));
			...
			$exim_section->set_title(__('Export/Import', 'alaya'));
